...
from src.config.settings import CITATION_MODEL
...
class CitationData(BaseModel):
    ...
    confidence_score: int = Field(
        description="Confidence score of verdict(0-100)", ge=0, le=100
    )
    contradictions: list[str] = Field(
        default=[], description="Sources that contradict this claim"
    )
    source_count: int = Field(description="Number of supporting sources found")
    contradiction_count: int = Field(
        description="Number of contradicting sources found"
    )
    source_quality: str = Field(
        description="peer-reviewed|official|mainstream|questionable"
    )
    is_controversial: bool = Field(description="Is this claim disputed among experts?")
    recency_score: str = Field(
        description="recent (<2yr)|moderate (2-5yr)|dated (>5yr)"
    )

    confidence_rationale: str = Field(
        description="Explain why this confidence score was assigned"
    )
...
class CitationsList(BaseModel):
    citations: list[CitationData] = Field(
        description="List of Citation Data for each claim"
    )
...
citation_agent_tool = citation_agent.as_tool(
    tool_name="fact_checker",
    tool_description="Fact-check the report and provide citations",
)
