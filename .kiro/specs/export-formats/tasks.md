...
- [ ] 3. Implement `MarkdownRenderer`
  ...
  - [x] 3.2 Write property test for `MarkdownRenderer` — Property 1: metadata completeness and document ordering
  ...
  - [x] 3.3 Write property test for `MarkdownRenderer` — Property 2: Q&A block count invariant
  ...
  - [x] 3.4 Write property test for `MarkdownRenderer` — Property 3: report body verbatim preservation
  ...
  - [x] 3.5 Write property test for `MarkdownRenderer` — Property 4: renderer determinism
    - **Property 4: Renderer determinism**
    - **Validates: Requirements 7.1**
    - File: `tests/test_property_export_renderers.py`
    - Call `render(parts)` twice; assert byte-for-byte identical output

  - [x] 3.6 Write property test for `MetadataHeader` title derivation — Property 5
  ...
  - [x] 3.7 Write property test for filename pattern — Property 6
...
- [x] 4. Implement `PDFRenderer`
  - [x] 4.1 Create `src/export/renderers/pdf.py` with a `render(parts: DocumentParts) -> bytes` function
  ...
  - [x] 4.2 Write property test for `PDFRenderer` — Property 8: valid PDF bytes
...
- [x] 5. Implement `Export_Service`
  - [x] 5.1 Create `src/export/service.py` with `async def export(session_id: UUID, fmt: ExportFormat, pool: asyncpg.Pool) -> ExportResult`
  ...
  - [x] 5.2 Write unit tests for `Export_Service` in `tests/test_unit_export_service.py`
...
- [x] 6. Implement `Export_Router`
  - [x] 6.1 Create `src/export/router.py` with `router = APIRouter(prefix="/export", tags=["export"])`
  ...
  - [x] 6.2 Write unit tests for `Export_Router` in `tests/test_unit_export_router.py`
  ...
  - [x] 6.3 Write property test for router — Property 7: invalid delivery_mode produces 422
  ...
  - [x] 6.4 Write property test for router — Property 9: no stack traces in error responses
...
- [x] 8. Register router and smoke tests
  - [x] 8.1 Register `Export_Router` in `src/api/main.py`
    - Add `from src.export.router import router as export_router`
    - Add `app.include_router(export_router, prefix="/api")`
    - _Requirements: 6.1_

  - [x] 8.2 Write smoke / integration tests in `tests/test_export_integration.py`
...
- [x] 10. Add export UI to Gradio thin client
  - [x] 10.1 Add export controls to `gradio_app.py` below the report output area
  ...
  - [x] 10.2 Implement `export_report` async handler in `gradio_app.py`
  ...
  - [x] 10.3 Wire the export button click event
  ...
  - [x] 10.4 Ensure the export controls are disabled / hidden when no session is active
