...
@function_tool
async def thorough_verify(
    claim: str, claim_context: str, background_research: str
) -> dict:
...
Remember: You can only LOWER or MAINTAIN the confidence score, never increase it."""
    ...
    try:
        ...
        final.search_queries_used = list(
            set(initial.search_queries_used + final.search_queries_used)
        )

        print(
            f"Red team complete: {initial.confidence_score} → {final.confidence_score}"
        )
...
async def group_verify(
    claims: list[str], shared_context: str, semantic_topic: str
) -> dict:
...
- Max 2 contradicting citations"""
    ...
    try:
        ...
        if verified_count < len(claims):
            print(
                f"⚠️  Group verification returned {verified_count}/{len(claims)} claims"
            )

        return {
            "verified_claims": [
                c.model_dump() for c in result.final_output.verified_claims
            ]
        }
        ...
        if any(keyword in error_str for keyword in ["JSON", "EOF", "parse", "truncat"]):
            print(
                "🔄 JSON truncation detected, falling back to individual quick verifications"
            )
            ...
            for claim in claims:
                try:
                    ...
                    print(
                        f"⚠️  Fallback verification also failed for claim: {claim[:50]}..."
                    )
            ...
            print(
                f"✓ Fallback complete: {len(individual_results)}/{len(claims)} claims verified individually"
            )
...
def parse_verification_result(
    result: dict,
) -> list[SingleClaimCitation] | SingleClaimCitation:
